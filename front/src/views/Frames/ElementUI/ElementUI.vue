...
export default {
  ...
  methods: {
    ...
    getList () {
      this.list = null;
      MyElementUI.GetElementUIList(this.postParams).then((res) => {
        // 模拟接口请求延时
        setTimeout(() => {
    ...
    dialogSave () {
      this.checkFormData().then(() => {
        this.isPushing = true;
        // 是否为编辑模式，id来判断
        MyElementUI.InsertElementUI(Object.assign({}, this.formData, { id: this.id })).then((res) => {
          setTimeout(() => {
    ...
    delItem (id) {
      this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
        ...
        console.log(MyElementUI);
        MyElementUI.DeleteElementUIList({ id })
          .then((res) => {
            this.dialogVisible = false;
            this.postParams = {
              Size: 20,
