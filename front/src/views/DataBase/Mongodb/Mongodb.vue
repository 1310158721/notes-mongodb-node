...
export default {
  ...
  methods: {
    ...
    getList () {
      this.list = null;
      Mongodb.GetMongodbList(this.postParams).then((res) => {
        // 模拟接口请求延时
        setTimeout(() => {
    ...
    dialogSave () {
      this.checkFormData().then(() => {
        this.isPushing = true;
        // 是否为编辑模式，id来判断
        Mongodb.InsertMongodb(Object.assign({}, this.formData, { id: this.id })).then((res) => {
          setTimeout(() => {
