...
class Es5List {
  constructor() {
    ...
    // Es5 数据 Model
    this.Es5Model = this.db.model('es5list', this.Es5Schema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.Es5Model.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.Es5Model.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    const Es5Model = this.Es5Model;
    const Es5ModelItem = new Es5Model(data);
    const res = await Es5ModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.Es5Model.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.Es5Model.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetEs5List() {
    this.app.get('/api/getEs5List', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddEs5ListItem() {
    this.app.post('/api/insertEs5ListItem', (req, res, next) => {
   ...
   */
  DeleteEs5ListItem () {
    this.app.get('/api/deleteEs5Item', (req, res, next) => {
   ...
   */
  DeleteAliOssPhotos() {
    console.log('Es5 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.Es5Model.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/Es5/').then((result) => {
          const OssHasPhotos = [];
  ...
  start() {
    this.GetEs5List();
    this.AddEs5ListItem();
    this.DeleteEs5ListItem();
    this.DeleteAliOssPhotos();
  }
}

const es5List = new Es5List();
es5List.start();