...
class DomList {
  constructor() {
    ...
    // Dom 数据 Model
    this.DomModel = this.db.model('domlist', this.DomSchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.DomModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.DomModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    const DomModel = this.DomModel;
    const DomModelItem = new DomModel(data);
    const res = await DomModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.DomModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.DomModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetDomList() {
    this.app.get('/api/getDomList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddDomListItem() {
    this.app.post('/api/insertDomListItem', (req, res, next) => {
   ...
   */
  DeleteDomListItem () {
    this.app.get('/api/deleteDomItem', (req, res, next) => {
  ...
  /**
   * 定时删除阿里云OSS对象上的多余图片文件（按块删除Dom/Fs等）
   */
  DeleteAliOssPhotos() {
    console.log('Dom 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.DomModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/Dom/').then((result) => {
          const OssHasPhotos = [];
  ...
  start() {
    this.GetDomList();
    this.AddDomListItem();
    this.DeleteDomListItem();
    this.DeleteAliOssPhotos();
  }
}

const domList = new DomList();
domList.start();