...
class MongodbList {
  constructor() {
    ...
    // Mongodb 数据 Model
    this.MongodbModel = this.db.model('mongodblist', this.MongodbSchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.MongodbModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.MongodbModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    const MongodbModel = this.MongodbModel;
    const MongodbModelItem = new MongodbModel(data);
    const res = await MongodbModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.MongodbModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.MongodbModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetMongodbList() {
    this.app.get('/api/getMongodbList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddMongodbListItem() {
    this.app.post('/api/insertMongodbListItem', (req, res, next) => {
   ...
   * 删除 Mongodb 列表
   */
  DeleteMongodbListItem () {
    this.app.get('/api/deleteMongodbItem', (req, res, next) => {
   ...
   */
  DeleteAliOssPhotos() {
    console.log('Mongodb 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.MongodbModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/Mongodb/').then((result) => {
          const OssHasPhotos = [];
          if (result.objects && result.objects.length) {
            ...
            myOss.deleteMulti(unExist).then(() => {
              console.log('Mongodb 相关的多余图片已删除');
            })
          }
        })
      });
  ...
  start() {
    this.GetMongodbList();
    this.AddMongodbListItem();
    this.DeleteMongodbListItem();
    this.DeleteAliOssPhotos();
  }
}

const mongodbList = new MongodbList();
mongodbList.start();