...
class UbuntuList {
  constructor() {
    ...
    // Ubuntu 数据 Model
    this.UbuntuModel = this.db.model('ubuntulist', this.UbuntuSchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.UbuntuModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.UbuntuModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    const UbuntuModel = this.UbuntuModel;
    const UbuntuModelItem = new UbuntuModel(data);
    const res = await UbuntuModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.UbuntuModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.UbuntuModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetUbuntuList() {
    this.app.get('/api/getUbuntuList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddUbuntuListItem() {
    this.app.post('/api/insertUbuntuListItem', (req, res, next) => {
   ...
   * 删除 Ubuntu 列表
   */
  DeleteUbuntuListItem () {
    this.app.get('/api/deleteUbuntuItem', (req, res, next) => {
  ...
  /**
   * 定时删除阿里云OSS对象上的多余图片文件（按块删除Ubuntu/Fs等）
   */
  DeleteAliOssPhotos() {
    console.log('Ubuntu 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.UbuntuModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/Ubuntu/').then((result) => {
          const OssHasPhotos = [];
  ...
  start() {
    this.GetUbuntuList();
    this.AddUbuntuListItem();
    this.DeleteUbuntuListItem();
    this.DeleteAliOssPhotos();
  }
}

const ubuntuList = new UbuntuList();
ubuntuList.start();