...
class VueCliList {
  constructor() {
    ...
    // VueCli 数据 Model
    this.VueCliModel = this.db.model('vueclilist', this.VueCliSchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.VueCliModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.VueCliModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    const VueCliModel = this.VueCliModel;
    const VueCliModelItem = new VueCliModel(data);
    const res = await VueCliModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.VueCliModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.VueCliModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetVueCliList() {
    this.app.get('/api/getVueCliList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddVueCliListItem() {
    this.app.post('/api/insertVueCliListItem', (req, res, next) => {
   ...
   * 删除 VueCli 列表
   */
  DeleteVueCliListItem () {
    this.app.get('/api/deleteVueCliItem', (req, res, next) => {
   ...
   */
  DeleteAliOssPhotos() {
    console.log('VueCli 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.VueCliModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/VueCli/').then((result) => {
          const OssHasPhotos = [];
  ...
  start() {
    this.GetVueCliList();
    this.AddVueCliListItem();
    this.DeleteVueCliListItem();
    this.DeleteAliOssPhotos();
  }
}

const vueCliList = new VueCliList();
vueCliList.start();