...
class EslintList {
  constructor() {
    ...
    // Eslint 数据 Model
    this.EslintModel = this.db.model('eslintlist', this.EslintSchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.EslintModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.EslintModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    const EslintModel = this.EslintModel;
    const EslintModelItem = new EslintModel(data);
    const res = await EslintModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.EslintModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.EslintModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetEslintList() {
    this.app.get('/api/getEslintList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddEslintListItem() {
    this.app.post('/api/insertEslintListItem', (req, res, next) => {
   ...
   * 删除 Eslint 列表
   */
  DeleteEslintListItem () {
    this.app.get('/api/deleteEslintItem', (req, res, next) => {
   ...
   */
  DeleteAliOssPhotos() {
    console.log('Eslint 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.EslintModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/Eslint/').then((result) => {
          const OssHasPhotos = [];
  ...
  start() {
    this.GetEslintList();
    this.AddEslintListItem();
    this.DeleteEslintListItem();
    this.DeleteAliOssPhotos();
  }
}

const eslintList = new EslintList();
eslintList.start();