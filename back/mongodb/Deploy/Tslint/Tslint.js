...
class TslintList {
  constructor() {
    ...
    // Tslint 数据 Model
    this.TslintModel = this.db.model('tslintlist', this.TslintSchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.TslintModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.TslintModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    const TslintModel = this.TslintModel;
    const TslintModelItem = new TslintModel(data);
    const res = await TslintModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.TslintModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.TslintModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetTslintList() {
    this.app.get('/api/getTslintList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddTslintListItem() {
    this.app.post('/api/insertTslintListItem', (req, res, next) => {
   ...
   * 删除 Tslint 列表
   */
  DeleteTslintListItem () {
    this.app.get('/api/deleteTslintItem', (req, res, next) => {
   ...
   */
  DeleteAliOssPhotos() {
    console.log('Tslint 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.TslintModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/Tslint/').then((result) => {
          const OssHasPhotos = [];
  ...
  start() {
    this.GetTslintList();
    this.AddTslintListItem();
    this.DeleteTslintListItem();
    this.DeleteAliOssPhotos();
  }
}

const tslintList = new TslintList();
tslintList.start();