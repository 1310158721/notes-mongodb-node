...
class LessList {
  constructor() {
    ...
    // Less 数据 Model
    this.LessModel = this.db.model('lesslist', this.LessSchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.LessModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.LessModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    const LessModel = this.LessModel;
    const LessModelItem = new LessModel(data);
    const res = await LessModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.LessModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.LessModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetLessList() {
    this.app.get('/api/getLessList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddLessListItem() {
    this.app.post('/api/insertLessListItem', (req, res, next) => {
   ...
   */
  DeleteLessListItem () {
    this.app.get('/api/deleteLessItem', (req, res, next) => {
  ...
  /**
   * 定时删除阿里云OSS对象上的多余图片文件（按块删除Less/Fs等）
   */
  DeleteAliOssPhotos() {
    console.log('Less 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.LessModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/Less/').then((result) => {
          const OssHasPhotos = [];
  ...
  start() {
    this.GetLessList();
    this.AddLessListItem();
    this.DeleteLessListItem();
    this.DeleteAliOssPhotos();
  }
}

const lessList = new LessList();
lessList.start();