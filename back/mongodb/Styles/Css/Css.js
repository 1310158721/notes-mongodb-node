...
class CssList {
  constructor() {
    ...
    // Css 数据 Model
    this.CssModel = this.db.model('csslist', this.CssSchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.CssModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.CssModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    ...
    const CssModelItem = new CssModel(data);
    const res = await CssModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.CssModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.CssModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetCssList() {
    this.app.get('/api/getCssList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddCssListItem() {
    this.app.post('/api/insertCssListItem', (req, res, next) => {
   ...
   */
  DeleteCssListItem () {
    this.app.get('/api/deleteCssItem', (req, res, next) => {
  ...
  /**
   * 定时删除阿里云OSS对象上的多余图片文件（按块删除Css/Fs等）
   */
  DeleteAliOssPhotos() {
    console.log('Css 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.CssModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/Css/').then((result) => {
          const OssHasPhotos = [];
  ...
  start() {
    this.GetCssList();
    this.AddCssListItem();
    this.DeleteCssListItem();
    this.DeleteAliOssPhotos();
  }
}

const cssList = new CssList();
cssList.start();