...
class FsList {
  constructor() {
    ...
    // Fs 数据 Model
    this.FsModel = this.db.model('fslist', this.FsSchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.FsModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.FsModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    ...
    const FsModelItem = new FsModel(data);
    const res = await FsModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.FsModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.FsModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetFsList() {
    this.app.get('/api/getFsList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddFsListItem() {
    this.app.post('/api/insertFsListItem', (req, res, next) => {
      const body = Object.assign(req.body, { CreatedTime: new Date() });
      const _id = body.id;
   ...
   */
  DeleteFsListItem () {
    this.app.get('/api/deleteFsItem', (req, res, next) => {
   ...
   */
  DeleteAliOssPhotos() {
    console.log('Fs 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.FsModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/Fs/').then((result) => {
          const OssHasPhotos = [];
  ...
  start() {
    this.GetFsList();
    this.AddFsListItem();
    this.DeleteFsListItem();
    this.DeleteAliOssPhotos();
  }
}

const fsList = new FsList();
fsList.start();