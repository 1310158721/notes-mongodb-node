...
class ElementUIList {
  constructor() {
    ...
    // ElementUI 数据结构 Schema
    this.ElementUISchema = new Schema({
      Title: String,
      Tag: [String],
      Desc: String,
      Content: String,
    ...
    // ElementUI 数据 Model
    this.ElementUIModel = this.db.model('elementuilist', this.ElementUISchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.ElementUIModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.ElementUIModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    const ElementUIModel = this.ElementUIModel;
    const ElementUIModelItem = new ElementUIModel(data);
    const res = await ElementUIModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.ElementUIModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.ElementUIModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetElementUIList() {
    this.app.get('/api/getElementUIList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   * 插入/更新单条数据
   */
  AddElementUIListItem() {
    this.app.post('/api/insertElementUIListItem', (req, res, next) => {
   ...
   * 删除 ElementUI 列表
   */
  DeleteElementUIListItem () {
    this.app.get('/api/deleteElementUIItem', (req, res, next) => {
   ...
   */
  DeleteAliOssPhotos() {
    console.log('ElementUI 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.ElementUIModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/ElementUI/').then((result) => {
          const OssHasPhotos = [];
          if (result.objects && result.objects.length) {
            ...
            myOss.deleteMulti(unExist).then(() => {
              console.log('ElementUI 相关的多余图片已删除');
            })
          }
        })
      });
  ...
  start() {
    this.GetElementUIList();
    this.AddElementUIListItem();
    this.DeleteElementUIListItem();
    this.DeleteAliOssPhotos();
  }
}

const elementUIList = new ElementUIList();
elementUIList.start();