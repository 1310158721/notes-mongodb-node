...
class VueList {
  constructor() {
    ...
    // Vue 数据 Model
    this.VueModel = this.db.model('vuelist', this.VueSchema);
  }
  ...
  async FindList(conditions = {}, Size, Page) {
    const res = await this.VueModel.find(conditions)
  ...
  async FindListCount(conditions = {}) {
    const count = await this.VueModel.find(conditions).countDocuments();
    return count;
  }
  ...
  async InsertListItem(data = {}) {
    const VueModel = this.VueModel;
    const VueModelItem = new VueModel(data);
    const res = await VueModelItem.save();
    return res;
  }
  ...
  async UpdateListItem(conditions = {}, updateData) {
    const res = await this.VueModel.updateOne(conditions, { $set: { ...updateData } })
    return res;
  }
  ...
  async DeleteListItem (conditions = {}) {
    const res = await this.VueModel.deleteOne(conditions);
    return res;
  }
   ...
   */
  GetVueList() {
    this.app.get('/api/getVueList', (req, res, next) => {
      const {
        Size = 10,
        Page = 1,
        keyword = '',
   ...
   */
  AddVueListItem() {
    this.app.post('/api/insertVueListItem', (req, res, next) => {
   ...
   */
  DeleteVueListItem () {
    this.app.get('/api/deleteVueItem', (req, res, next) => {
   ...
   */
  DeleteAliOssPhotos() {
    console.log('Vue 阿里云OSS checking____________________________________________');
    let timer = null;
    // 设置定时器
    timer = setInterval(() => {
    // 查找对应数据库获取当前正在用的所有图片地址
    this.VueModel.find({}).then((doc) => {
      let mongodbPhotos = '';
      doc.map((i) => {
      ...
      myOss.setBuckName('tanglihe-notes').then(() => {
        myOss.listDir('list-content/Vue/').then((result) => {
          const OssHasPhotos = [];
  ...
  start() {
    this.GetVueList();
    this.AddVueListItem();
    this.DeleteVueListItem();
    this.DeleteAliOssPhotos();
  }
}

const vueList = new VueList();
vueList.start();